...
1.	Let $d$ be the fraction of population that dies each year. What is $d$ for the ring-tailed pheasant population on Protection Island? Write down any assumptions you have made. [3 marks]

4.	$b$ is the per capita number of births each year. What is the value of $b$? Write down any assumptions you have made. [3 marks]

5.	Recall that $\lambda = 1 + b-d$. What is the value of $\lambda$? Is this population is expected to grow over time? [2 marks]
...
Write an R scipt that builds on the file you have previously made _protection-island.R_. Use the `lines()` command to add the predicted population size assuming geometric growth using the commands described in this section. If you have written the code correctly the result should look something like this:
