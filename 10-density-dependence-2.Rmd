# Fri Oct 2: Solving the logistic growth equation using a computer 1
...
where $y(x)$ is the dependent variable, the derivative of $y(x)$ is with respect to $x$, and $x$, the independent variable, appears on the opposite side of the `=` to the derivative of the dependent variable. In the notation of population biology, where $N(t)$ is conventional for the dependent variable, a similar problem might look like this:
...
`y` are the initial values of the dependent varaible.

`times` is a sequence for which output is wanted; the first value of times is the initial time.

`func` is an R-function that computes the values of the derivatives of the dependent variables. `func` must be defined as: `func <- function(t, y, parms,...)`. $t$ is the current time point in the integration, y is the current estimate of the variables in the ODE system.
...
The above code is fine, but since we've done the same calculation twice (conversion of Celcius to Farenheit), just with a different number, we might want to consider writing a custom function. Let's re-write the above code so that it now uses a custom function:
...
## Questions
1. The following is an equation for exponential population growth, but with a constant immigration rate, $m >0$, into the population:
